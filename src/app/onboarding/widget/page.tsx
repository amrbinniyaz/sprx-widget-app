...
import { ArrowRight, ArrowLeft, LayoutGrid, Columns as MasonryIcon, Layers, Rows, Heart, MessageCircle, Settings, Paintbrush, BoxSelect, Copy, CheckCircle2, ChevronRight, Share2, Monitor, Smartphone, Link2, AppWindow, Image as ImageIcon } from "lucide-react";
...
const mockPosts = [
  { id: 1, author: "eat_for_pleasure", time: "March 30, 2026", text: "Love is like the wind, you can't see it but you can feel it.", likes: 204, comments: 12, image: "https://images.unsplash.com/photo-1492684223066-81342ee5ff30?w=600&q=80", icon: "instagram", aspect: "aspect-[4/5]" },
  { id: 2, author: "baking_magic", time: "April 2, 2026", text: "The perfect chocolate swirl cake is all about the right icing technique.", likes: 856, comments: 42, image: "https://images.unsplash.com/photo-1551601651-2a8555f1a136?w=600&q=80", icon: "instagram", aspect: "aspect-square" },
  { id: 3, author: "travel_diaries", time: "April 5, 2026", text: "Barcelona skyline hits different from up here. Architectural masterpiece.", likes: 1240, comments: 89, image: "https://images.unsplash.com/photo-1522202176988-66273c2fd55f?w=600&q=80", icon: "tiktok", aspect: "aspect-[3/4]" },
  { id: 4, author: "arch_digest", time: "April 8, 2026", text: "Look closely at the incredibly detailed ceiling of the Pantheon.", likes: 3120, comments: 156, image: "https://images.unsplash.com/photo-1517457373958-b7bdd4587205?w=600&q=80", icon: "x", aspect: "aspect-[4/5]" },
  { id: 5, author: "event_planner", time: "April 10, 2026", text: "Sparklers make every celebration grand! 🎇", likes: 890, comments: 24, image: "https://images.unsplash.com/photo-1540317580384-e5d43867caa6?w=600&q=80", icon: "facebook", aspect: "aspect-[16/9]" },
  { id: 6, author: "design_inspo", time: "April 12, 2026", text: "Sleek workspace minimal setup. What do you think?", likes: 450, comments: 18, image: "https://images.unsplash.com/photo-1526406915894-7bcd65f60845?w=600&q=80", icon: "pinterest", aspect: "aspect-[4/3]" },
];
...
export default function WidgetPage() {
  ...
  const getContainerClass = () => {
    ...
    if (device === "mobile") {
       return config.layout === "masonry" ? "columns-1 gap-5 space-y-5" : "grid grid-cols-1 gap-5";
    }
  ...
  return (
    <div className="max-w-[1400px] mx-auto flex flex-col justify-center min-h-[calc(100vh-8rem)]">
      
      <div className="flex justify-between items-end mb-6 animate-fade-in-up">
        <div>
           <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-md bg-zinc-900 border border-zinc-700 text-zinc-100 text-[10px] font-bold uppercase tracking-wider mb-3">
             Step 4: Editor
           </div>
      ...
      <div className="flex flex-col md:flex-row h-[75vh] min-h-[600px] w-full rounded-2xl overflow-hidden shadow-2xl border border-zinc-200/60 bg-white animate-fade-in-up delay-100 relative z-20">
        
        {/* Far-left slim sidebar */}
        <div className="w-[72px] bg-zinc-950 flex flex-col items-center py-6 gap-6 border-r border-zinc-800 z-10 shrink-0">
           {[
             ...
             <button 
               ...
               className={`flex flex-col items-center gap-1.5 p-2 transition-all group w-full ${activeTab === tab.id ? 'text-blue-500 opacity-100' : 'text-zinc-500 opacity-70 hover:opacity-100 hover:text-zinc-300'}`}
             >
        ...
        {/* Configuration Panel */}
        <div className="w-full md:w-[320px] bg-[#2A2B2D] text-zinc-100 flex flex-col overflow-y-auto border-r border-zinc-800 custom-scrollbar z-10 shadow-xl shrink-0">
             ...
             {activeTab === "sources" && (
                <div className="space-y-6 animate-fade-in-up">
                   ...
                   <div className="space-y-3">
                     {[
                       { id: "ig", name: "Instagram", user: "@eat_for_pleasure", color: "from-pink-500 to-amber-500", icon: "IG" },
                       { id: "tt", name: "TikTok", user: "@travel_diaries", color: "from-zinc-900 to-cyan-500", icon: "TK" },
                       { id: "x", name: "X (Twitter)", user: "@arch_digest", color: "from-zinc-700 to-zinc-900", icon: "X" }
                     ].map(source => (
                       <div key={source.id} className="bg-[#1C1C1D] border border-zinc-800 rounded-xl p-3 flex items-center justify-between group cursor-pointer hover:border-zinc-700 hover:bg-[#202123] transition-colors shadow-sm">
                         <div className="flex items-center gap-3">
                           <div className={`w-9 h-9 rounded-lg bg-gradient-to-br ${source.color} flex items-center justify-center shadow-inner`}>
                             <span className="text-white text-[11px] font-bold tracking-tight">{source.icon}</span>
                           </div>
                           <div>
                   ...
                   {layouts.map(l => {
                     ...
                     return (
                       <button
                         ...
                         className={`flex flex-col items-center justify-center relative group p-3 rounded-[4px] border-2 transition-all gap-1.5 ${
                       ...
                       >
                         {isActive && (
                           <div className="absolute -top-2.5 -right-2.5 w-[20px] h-[20px] bg-blue-500 rounded-full border-[3px] border-[#2A2B2D] flex items-center justify-center z-10">
                              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="4" className="w-[10px] h-[10px] text-white stroke-white"><path d="M20 6L9 17l-5-5" strokeLinecap="round" strokeLinejoin="round"/></svg>
                           </div>
                         )}
                ...
                <div className="space-y-6 animate-fade-in-up">
                   ...
                   <div className="bg-[#1C1C1D] border border-zinc-800 rounded-xl p-4">
                     ...
                     <div className="bg-black/50 overflow-x-auto rounded-md p-3 mb-3 border border-zinc-800/50">
                        <pre className="text-[10px] text-zinc-500 font-mono"><code dangerouslySetInnerHTML={{ __html: embedCode.replace(/</g, "&lt;").replace(/"([^"]*)"/g, '<span class="text-blue-400">"$1"</span>') }} /></pre>
                     </div>
        ...
        {/* Live Preview Canvas */}
        <div className="flex-1 bg-[#F5F7FA] relative flex flex-col items-center z-0">
           ...
           {/* Preview Container */}
           <div className="w-full h-full overflow-y-auto px-4 py-24 custom-scrollbar">
                   ...
                   {mockPosts.slice(0, config.layout === "slider" ? 1 : 12).map((post, i) => (
                      <div 
                        ...
                        className={`bg-white rounded-[20px] overflow-hidden shadow-[0_8px_30px_rgba(0,0,0,0.06)] hover:shadow-[0_12px_40px_rgba(0,0,0,0.1)] transition-shadow group flex flex-col ${
                          ...
                          (config.layout === "carousel" || config.layout === "slider") ? `min-w-[85%] ${config.layout === "carousel" ? "md:min-w-[45%]" : "md:min-w-[100%]"} flex-shrink-0 snap-center` : ""
                        }`} 
                      ...
                      >
                         ...
                         <div className={`relative ${config.layout === "slider" ? "aspect-[2/1]" : post.aspect} overflow-hidden w-full bg-zinc-100`}>
                           ...
                           {/* Overlay overlay */}
                           <div className="absolute bottom-4 left-4 right-4 flex flex-col justify-end text-white relative z-10 pointer-events-none">
                              {config.showCaption && (
                         ...
                         <div className="flex items-center justify-between p-4 bg-white border-t border-zinc-100">
                            <div className="flex gap-4">
                              <span className="flex items-center gap-1.5 text-[12px] font-bold text-zinc-700 cursor-pointer hover:text-rose-500 transition-colors"><Heart size={14} className="stroke-2" /> {post.likes}</span>
                              <span className="flex items-center gap-1.5 text-[12px] font-bold text-zinc-700 cursor-pointer hover:text-blue-500 transition-colors"><MessageCircle size={14} className="stroke-2" /> {post.comments}</span>
                            </div>
                            <span className="flex items-center gap-1.5 text-[12px] font-bold text-zinc-500 hover:text-zinc-900 cursor-pointer transition-colors"><Share2 size={13} className="stroke-2" /> Share</span>
                         </div>
                      </div>
                   ))}
                 </div>
